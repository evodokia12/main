...
local function GetPlayerFromString(str, ignore)
    for i, Targ in pairs(game.Players:GetPlayers()) do 
        if not (not ignore and Targ == Variables["Player"]) then
...
local SetupsTable = {
  ...
  Ufo = {
    Origin = CFrame.new(112.918541, 138.999954, -617.407532, -0.0103775486, 7.93669415e-08, 0.999946177, 7.76252804e-08, 1, -7.85656127e-08, -0.999946177, 7.6805776e-08, -0.0103775486),
    ZMultiplier = -8,
    XMultiplier = 10,
    PerRow = 10,
    Rows = 4,
	...
	Klub = {
    Origin = CFrame.new(-294.051605, -6.20834684, -407.35907, 0.999949634, 1.26443012e-09, -0.010037058, -1.04804643e-09, 1, 2.1563741e-08, 0.010037058, -2.15521361e-08, 0.999949634),
    ZMultiplier = 8,
    XMultiplier = 12,
    PerRow = 8,
    Rows = 6,
	...
	Train = {
    Origin = CFrame.new(678.116272, 47.9999886, -101.561012, -0.999894381, 1.01588393e-09, 0.0145345135, 1.79814097e-09, 1, 5.38075895e-08, -0.0145345135, 5.38280389e-08, -0.999894381),
    ZMultiplier = -8,
    XMultiplier = 10,
    PerRow = 6,
    Rows = 8,
	...
	if Debugmode then
		for PointInTable = 1,40 do
			...
			elseif PointInTable <= 40 then
				i = 4
            elseif PointInTable <= 50 then
				i = 5
            elseif PointInTable <= 60 then
				i = 6
            elseif PointInTable <= 70 then
				i = 7
            elseif PointInTable <= 80 then
				i = 8
            elseif PointInTable <= 90 then
				i = 9
			end
			if i == 1 then
